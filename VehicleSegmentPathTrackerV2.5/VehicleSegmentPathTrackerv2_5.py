#This program does vehicle detection, segmentation, and tracking using YOLOv11m, YOLO11m-seg and DeepSORT Tracker
#with MobileNetv2, from the Trackers library. It uses a stacked detections and segmentations algorithm 
#with morphological operations, and applies various optimizations for good performance and accuracy.
...
#This function loads an image from a file path and converts it to a numpy array
class SegmentedVehiclePathTracker:
    
    #Initialize the tracker with model paths and parameters.
    def __init__(self, yolo_model_path=r"C:\Users\Xeron\OneDrive\Documents\Programs\RoadVehiclesYOLODatasetProTraining\RoadVehiclesYOLODatasetPro_TrainingOutput\train\weights\RoadVehiclesYOLO11m.pt", 
        ...
        #Initialize feature extractor for DeepSORT using a pretrained model.
        import torch
        pretrained_model = torch.hub.load('pytorch/vision:v0.10.0', 'mobilenet_v2', weights='MobileNet_V2_Weights.DEFAULT')
        feature_extractor = DeepSORTFeatureExtractor(
            model_or_checkpoint_path=pretrained_model,
            device=device,
            input_size=(128, 128)
        )
    ...
    #Validate and adjust bounding box coordinates to be within frame dimensions.    
        ...
        for i, detection in enumerate(detections):
            ...
            if roi.size > 0:
                try:
                    ...
                    seg_results = self.segmentation_model(roi, conf=seg_conf, iou=iou_thresh, classes=[0, 1, 2, 3, 4, 5, 6, 7], verbose=False)
    ...
    #Estimate a vehicle mask from a bounding box using edge detection and morphological operations.
    ...
    #Process a single video frame: detect, segment, track vehicles, and update trails.
    def process_frame(self, frame):
        ...
        try:
            tracked_detections = self.tracker.update(detections=detections_sv, frame=frame)
        except Exception as e:
        ...
        if len(tracked_detections) > 0 and tracked_detections.tracker_id is not None:
            for i in range(len(tracked_detections)):
                if (tracked_detections.tracker_id[i] is not None and 
                    tracked_detections.tracker_id[i] >= 0):  # Only include valid track IDs
                    xyxy = tracked_detections.xyxy[i]
                    conf = tracked_detections.confidence[i]
                    class_id = tracked_detections.class_id[i]
                    track_id = tracked_detections.tracker_id[i]
                    tracks.append([xyxy[0], xyxy[1], xyxy[2], xyxy[3], track_id, class_id, conf])
    ...
    #Get a distinct color for each track ID for visualization.
    def get_track_color(self, track_id):
        
        #Use a fixed palette of distinct colors.
        colors = [
    ...
    #Update the trail for a specific track ID with the new center point.
    def update_trail(self, track_id, center):
        try:
            track_id = int(track_id)
        except (ValueError, TypeError):
            print(f"[Warning] Invalid track_id: {track_id}")
            return
        
        if (not isinstance(center, tuple) or len(center) != 2 or
            not isinstance(center[0], (int, float)) or not isinstance(center[1], (int, float))):
            print(f"[Warning] Invalid center coordinates for track {track_id}: {center}")
            return
    ...
    #Update the segment masks for a specific track ID.
    ...
    def cleanup_old_tracks(self, current_track_ids):
        ...
        current_ids = set(int(tid) for tid in current_track_ids if isinstance(tid, (int, float)))
        
        old_trail_ids = set(self.track_trails.keys()) - current_ids
        old_segment_ids = set(self.track_segments.keys()) - current_ids  
        old_mask_ids = set(self.previous_masks.keys()) - current_ids

        for old_id in old_trail_ids:
            if old_id in self.track_trails:
                del self.track_trails[old_id]
                
        for old_id in old_segment_ids:
            if old_id in self.track_segments:
                del self.track_segments[old_id]
                
        for old_id in old_mask_ids:
            if old_id in self.previous_masks:
                del self.previous_masks[old_id]
    
    #Reset the tracking state, clearing all trails and segments.
    ...
    #Draw tracking results, bounding boxes, masks, and trails on the frame.
            ...
            if len(track) >= 5:
                ...
                if bbox is not None:
                    ...
                    try:
                        ...
                        print(f"[Warning] Invalid track ID: {track[4]}")
                else:
        ...
        for track in valid_tracks:
            if len(track) >= 5:
                ...
                if len(trail_points) > 1:
                    current_trail = self.track_trails[track_id]
                    
                    for i in range(1, len(current_trail)):
                        pt1 = current_trail[i-1]
                        pt2 = current_trail[i]
                        
                        if (isinstance(pt1, tuple) and len(pt1) == 2 and 
                            isinstance(pt2, tuple) and len(pt2) == 2 and
                            isinstance(pt1[0], (int, float)) and isinstance(pt1[1], (int, float)) and
                            isinstance(pt2[0], (int, float)) and isinstance(pt2[1], (int, float))):
                            
                            pt1_safe = (int(pt1[0]), int(pt1[1]))
                            pt2_safe = (int(pt2[0]), int(pt2[1]))
                            
                            if (0 <= pt1_safe[0] <= w and 0 <= pt1_safe[1] <= h and
                                0 <= pt2_safe[0] <= w and 0 <= pt2_safe[1] <= h):
                                cv2.line(annotated_frame, pt1_safe, pt2_safe, color, 2)
    ...
    #Process the entire video: read frames, process, annotate, and write output video.
...
# Command line interface for the vehicle path tracker.
def main():
    #Parse command line arguments.
    parser = argparse.ArgumentParser(description="Enhanced Vehicle Path Tracker with DeepSORT")
    ...
    print("Enhanced Vehicle Path Tracker with DeepSORT")
    print("=" * 60)
